...
// requests. You will not be able to send different request with different proxy unless you change your `http_proxy`
// environment again. Another example is using Golang proxy setting. This is normal prefer way to do but too verbase
// compared to GoRequest's Proxy:
//
...
// parameter. Its duty is to append slice of interface{} into s.SliceData ([]interface{}) which later changes into json
// array in the End() func.
...
// parameter. Its duty is to transfrom interface{} (implicitly always a struct) into s.Data (map[string]interface{})
...
// file is a string (path) or a os.File. When file is a []byte slice, filename defaults to "filename". In all cases the
// automatically determined filename can be overwritten:
//
//      b, _ := ioutil.ReadFile("./example_file.ext")
//      gorequest.New().
//        Post("http://example.com").
...
// defaults to fileNUMBER (eg. file1), where number is ascending and starts counting at 1. So if you send multiple
// files, the fieldnames will be file1, file2, ... unless it is overwritten. If fieldname is set to "file" it will be
// automatically set to fileNUMBER, where number is the greatest exsiting number+1.
//
//      b, _ := ioutil.ReadFile("./example_file.ext")
//      gorequest.New().
//        Post("http://example.com").
...
// calling it. End function returns Response which matchs the structure of Response type in Golang's http package (but
// without Body data). The body data itself returns as a string in a 2nd return value. Lastly but worth noticing, error
// array (NOTE: not just single error value) is returned as a 3rd value and nil otherwise.
//
// For example:
//
