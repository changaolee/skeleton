...
skt::log::usage_from_stdin() {
  local messages=()
  while read -r line; do
    messages+=("${line}")
  done

  skt::log::usage "${messages[@]}"
}

# Print out some info that isn't a top level status line
skt::log::info() {
  local V="${V:-0}"
  if [[ ${SKT_VERBOSE} < ${V} ]]; then
    return
  fi
...
skt::log::info_from_stdin() {
  local messages=()
  while read -r line; do
    messages+=("${line}")
  done

  skt::log::info "${messages[@]}"
}
