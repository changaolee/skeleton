...
2. 执行命令： ./wrktest.sh diff skeleton.dat http.dat
> Note: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

sktroot="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
...
OPTIONS:
  -h                     Usage information
  -n                     Performance test task name, default: skeleton
  -d                     Directory used to store performance data and gnuplot graphic, default: _output/wrk
Report bugs to <changao.li.work@outlook.com>.
EOF
}
